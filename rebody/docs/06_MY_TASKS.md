...
M1.6 포지셔닝 확장         ◀ 신설 — 전 생활패턴 커버 (M1.5와 같이 진행)
M2  백엔드 배포
...
## M1.6 — 포지셔닝 확장 ◀ **신설** (M1.5와 함께 진행)

**2026-07-28 결정: 타깃은 교대근무자가 아니라 모든 생활 패턴입니다.**
제품은 **간헐적 단식 스케줄링**이고, 차별점은 "사용자의 사이클을 이해한다"입니다.
교대근무는 가장 어려운 케이스로서 신뢰의 근거로만 쓰고, 전면 카피는 일반 사용자 언어로 갑니다.

### 다행히 엔진은 이미 범용입니다

`pattern_type`이 `fixed_weekly` / `rotating_cycle` / `irregular` 3종이고 `cycle_length_days`가 가변이라,
1일(매일 동일) · 7일(요일별) · 6일·8일(교대) 사이클이 **같은 코드로 표현됩니다.**
타임존도 `users.timezone` 컬럼 기반이라 서울 고정이 아닙니다. 즉 **바꿔야 할 것은 스키마가 아니라 프리셋과 언어**입니다.

| # | 할 일 | 종류 |
|---|---|---|
| 1.6-1 | **프리셋 확장** — 현재 `day_fixed`/`night_fixed`/`two_shift`/`three_shift` 4종은 전부 "근무" 전제. 아래를 추가 | 코드 |
| 1.6-2 | 프리셋 그룹핑 UI — "생활 패턴을 골라주세요" 상위에 `일반 / 불규칙 / 교대 / 출장·시차` 4개 그룹 | 코드 |
| 1.6-3 | `PresetCycleDay`의 `work_start`/`work_end` → 의미를 **"활동 시작/종료"**로 재정의 (필드명 유지, 라벨만 변경) · 은퇴·재택은 기상/취침으로 표시 | 코드 |
| 1.6-4 | **타임존 변경 플로우** — 출장자 대응. 설정에서 TZ 변경 시 "①현지 시각에 맞춰 재배치 ②한국 시각 유지" 선택 + 스케줄 재계산 | 코드 |
| 1.6-5 | `parse-schedule-nl` 프롬프트에서 근무 전제 제거 ("수업", "알바", "운동", "취침" 어휘 수용) | 코드 |
| 1.6-6 | 온보딩·대시보드 카피에서 "근무" → "활동/일정" 치환 | 카피 |
| 1.6-7 | 스토어 문구(M6-4) 1순위 문장을 일반 사용자 기준으로 작성. 교대근무는 두 번째 문단의 근거로 | 카피 |
| 1.6-8 | ASO 키워드 재설정 — `간헐적단식 / 16:8 / 공복 / 다이어트 식단`을 1순위, `교대근무 / 야간근무`는 롱테일 | 스토어 |

### 추가할 프리셋 (안)

| 그룹 | 프리셋 | 사이클 | 앵커 질문 |
|---|---|---|---|
| 일반 | **매일 같은 리듬** (기존 앱과 동일한 기본형) | 1일 | "보통 몇 시에 일어나세요?" |
| 일반 | 주간 직장인 (기존 `day_fixed`) | 7일 | "이번 주 월요일 날짜는?" |
| 일반 | **학생·수험생** (등교/수업 + 야간 자습) | 7일 | "평일 등교 시각은?" |
| 불규칙 | **알바·프리랜서** (요일별로 다름 → 요일마다 직접 입력) | 7일 | "요일별 일정을 알려주세요" |
| 불규칙 | **은퇴·재택** (고정 일정 없음 → 기상/취침만으로 창 배치) | 1일 | "기상·취침 시각은?" |
| 교대 | 야간 고정 / 2교대 / 3교대 (기존 3종) | 7·6·8일 | 기존 유지 |
| 출장 | **시차 적응 모드** — 도착 타임존 + 도착일 입력 → 이동일은 단식 창을 늘려 완충 | 가변 | "언제, 어디로 가세요?" |

> **1.6-1의 판단 기준**: 프리셋이 많아지면 선택 비용이 늘어납니다. 첫 화면에는 **`매일 같은 리듬`을 기본 선택**으로 두고
> 나머지는 "내 패턴은 좀 달라요"로 접어두세요. 시장 대다수는 일반형이고, 교대·출장은 *차별점을 증명하는* 선택지입니다.

> **시차 프리셋 주의**: `cycle.ts`는 타임존을 모르는 civil-date 연산이고 변환은 `FastingScheduler`에서만 일어납니다.
> 이 경계를 지키는 한 TZ 변경은 "표시 계층 재계산"으로 끝납니다. 사이클 인덱스 계산에 TZ를 끌어들이면 하루가 밀립니다.

**게이트**: 프리셋 선택 화면에서 "나는 교대근무자가 아닌데?" 라는 인상이 들지 않을 것.
일반 사용자가 앵커 질문 1개만 답하고 스케줄을 받을 수 있을 것.
...
| M1.6 착수 시 | "프리셋 확장해줘" 한마디 | 생활패턴 프리셋 추가 + 타임존 변경 플로우 + 카피 치환 |
