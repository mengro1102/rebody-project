...
//
// 설계 의도:
//   자유 텍스트만으로 일정을 완전 자동 파싱하는 건 MVP 범위 밖이다(V2 이연).
//   여기서는 "이미 선택된 프리셋"을 기준선으로 두고, 사용자의 한 문장 보정만 반영한다.
//   모델이 처음부터 만들어 내는 게 아니라 diff만 내게 하므로 출력 토큰이 짧고 오류도 적다.
//
// 프롬프트는 근무 전제를 두지 않는다 — 사용자는 학생·알바·프리랜서·은퇴자일 수 있다.
// 확신이 낮으면(<0.6) 스케줄을 확정하지 않고 되묻는다. 일정을 잘못 잡으면
...
const SYSTEM_INSTRUCTION = `당신은 사용자의 하루 일정을 구조화하는 도우미입니다.

사용자는 교대근무자일 수도, 주간 직장인·학생·알바·프리랜서·은퇴자·재택 근무자일 수도 있습니다.
"근무"만 다루지 마세요. 수업, 등교, 알바, 시험, 운동, 육아, 기상·취침, 약속처럼
하루를 차지하는 활동은 모두 work_start / work_end 구간으로 표현합니다
(컬럼 이름은 work_*지만 의미는 "활동"입니다).
...
- 밤샘 근무·야간 자습처럼 자정을 넘기는 경우 종료 시각이 시작 시각보다 작게 표기됩니다
  (예: 22:00~06:00). 이는 정상이며 그대로 두세요.
- 고정 일정이 없는 날(은퇴·재택·휴무)은 work_start/work_end를 null로 두고
  식사 가능 시간대만 조정하세요. 활동이 없다고 해서 is_off_day를 임의로 바꾸지 마세요.
- 식사 가능 시간대(meal window)는 활동·운동 시간과 충돌하지 않게, 기상 후 활동 시간대
  안에 8~10시간 범위로 제안하세요.
